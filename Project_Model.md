...
- **Import latency (~12s at 146 events, ~7s at 1 event) — cause not yet established.** `match_events` is ~73% of server time: ~7.8s at 146 events (4 concurrent batches of 50) and ~1.9s for a *single* event. The initial analysis attributed the 1.9s floor to a large ticket prompt, but that assumed the 500-ticket cap was binding; real counts are ~89–139 per project, so the ticket block is only ~3–5k tokens and **most of the 1.9s is unexplained**. Do not act on the earlier estimates. Remaining candidates: Haiku's own latency floor, the fresh `new Anthropic()` per invocation (`ai-matcher.ts:99`, no connection reuse), TLS/cold connection, Vercel→API network. Next step is per-phase instrumentation *inside* `matchEvents` rather than further inference. Low-risk wins still available regardless: hoist the Anthropic client to module scope; prompt caching on the ticket block; compact the `JSON.stringify(…, null, 2)` serialisation (smaller payoff than first estimated). Ticket-list trimming is moot — the cap never binds.
- **AI quota is overcounted.** `eventsToMatch` (`sync/route.ts` ~181) bills all non-skipped events via `consumeAiUsage`, but only the `unmatched` subset reaches the model — events matched by key, mapping rule or learned history are charged without an AI call. Overcharges against the 200/month trial limit. Not yet fixed.
...
- **No test runner exists** — no `test` script, framework, or test files, while Section 8 sets testing expectations. The `import_run_analytics` bug (a silent write failure surviving weeks) is exactly the class of defect a test would have caught.
