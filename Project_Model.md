...
- **Ops:** Vercel hosting + Analytics; self-hosted monitoring via `captureAppError` → `app_system_events` → `/admin/system-events` + daily Resend digest. (Sentry was removed 2026-06 — see §6 decision log.)
...
- 2026-08-03 — **Pre-launch monitoring is the self-hosted stack, not Sentry, and a daily digest cadence is accepted for beta.** Sentry was removed in 2026-06 but four docs still named it, including a P0 launch blocker (`PRD.md`) that was therefore unachievable as written. That P0 is now **split in two**: (a) *spend/usage caps* — Supabase + Anthropic configured 2026-07-12, **Vercel deliberately still deferred** because its spend controls are weaker than the other two and Hobby-tier beta volume is low-risk; and (b) *error alerting* — `captureAppError` → `app_system_events` → `/admin/system-events` + daily Resend digest. The digest is **daily, not real-time**, because Vercel Hobby only permits daily cron; accepted for beta, with an external scheduler backlogged if tighter alerting is wanted. Launch-day monitoring therefore points at `/admin/system-events` directly, since a daily digest cannot surface a problem in time to act on it. — recorded in `PRD.md` §9, `LAUNCH_PLAN.md` Week 5.
- 2026-08-03 — **`refundAiUsage` throws on RPC failure instead of swallowing it**, and every caller catches it locally. A swallowed failure left the user charged quota they should have got back *and* let the route emit an `ai_usage_refunded` success event asserting a refund that never happened — worse than silence, since it would be trusted in a quota dispute. Callers inside existing `catch` blocks are individually guarded so a refund failure can never mask the error that actually broke the import, nor skip `failImportRun`.
