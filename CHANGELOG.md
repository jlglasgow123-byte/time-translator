...
- **The weekly security report now says why it failed (internal)** — The report has failed on every run since it was built three weeks ago, but the step that posts results to the app used `curl -sf`, which discards the server's response — so the only evidence was "exit code 22" with no reason. It now prints the HTTP status and response body, checks the secret is present before calling, and names the two likely causes explicitly (a `CRON_SECRET` mismatch, or a failure in the email send itself). No email had reached the email provider at all, so the report was failing well before the send.
