...
- **Contact form now asks for a summary, which becomes the email subject** — Every enquiry previously arrived with the identical subject "New message from Time Translator contact form", so replies read "Re: New message from Time Translator contact form" and nothing in the inbox distinguished one message from another. The form now has a required Summary field that is used as the subject line. The page copy was also rewritten to say the form is for general questions, and to point out that bugs and feature requests are better raised through the "?" icon in the bottom right of the screen.
...
- **Fixed the daily error alert, which had never once run (internal)** — The nightly job that emails a digest of production errors was being redirected to the sign-in page before it could run, because its address was missed from the list of routes allowed to work without a signed-in user. It has therefore never sent an email since it was built in July, and no error in production has ever triggered an alert. The address is now allowed, and that list has been restructured from a single unreadable line into a labelled list — the omission was invisible in the original form — with a note that cron routes must be added to it. The routes themselves were never unprotected: each checks its own secret.
- **The weekly security report is now working, after failing silently for three weeks (internal)** — Every run since it was built had failed, with no usable explanation: the step that posts results to the app used `curl -sf`, which discards the server's response, so the only evidence was "exit code 22". The actual cause was that the `CRON_SECRET` used to authenticate the workflow had never been added on the GitHub side — only in Vercel — and a missing secret is substituted as an empty string, so the request was silently unauthorised. The secret is now set in both places and the report is confirmed arriving with a correct pass/fail summary. The workflow also now prints the HTTP status and response body on failure and stops early with a clear message if the secret is missing, so a future failure is diagnosable rather than mute.
- **The contact page is now actually reachable** — The contact form existed but was unusable: logged-out visitors were redirected to the sign-in page, and nothing in the site linked to it except the sign-in page itself. That is exactly backwards, since the people most likely to need it are the ones who cannot get in. It is now public (no sign-in required), and reachable from the top nav, the footer, and a "still stuck?" link at the bottom of the Help page. The footer keeps the direct email address as an alternative.
- **Email sending is now configured and pointed at the verified domain** — Resend is set up, so the contact form, the weekly security report, error digests and promo/referral notifications can actually send; until now they failed silently. The sending domain was verified as the `contact.` subdomain rather than the bare domain, but all five senders had the bare domain hardcoded, which Resend would have rejected — so the address now lives in one place (`EMAIL_FROM`) and can be changed with an environment variable instead of edits in five files. Also added the previously missing `RESEND_API_KEY` to the example environment file, which is part of why this was assumed configured when it never had been.
- **A failed AI-quota refund is no longer invisible — or reported as a success** — When the AI service failed on our side, your quota was meant to be credited back. If that credit-back itself failed, nothing anywhere recorded it: you stayed charged for something you never received, and the system logged a "refund successful" note against a refund that never happened — which is worse than silence, since it would be believed if you ever queried your usage. Failures are now recorded properly and only genuine refunds are reported as such. A failed refund still never interrupts your import, and can no longer hide the underlying error that caused it.
- **Monitoring documentation now matches reality (internal)** — Four planning documents still named Sentry, an error-monitoring service removed back in June, including one pre-launch requirement that was therefore impossible to satisfy as written. That requirement is now split into the two separate things it had been conflating: spend caps (Supabase and Anthropic are configured; Vercel is deliberately deferred, with the reasoning recorded) and error alerting. The launch-day checklist also said to watch Sentry — it now points at the live admin events view instead, because the error digest only sends once daily and would not surface a launch-day problem in time to act on it.
- **Security: all nine outstanding dependency vulnerabilities resolved** — Updated Next.js (15.5.18 → 15.5.21) and PostCSS (8.5.15 → 8.5.25), clearing all nine open advisories, four of them rated high. The high-severity ones covered two server-side request forgery paths, a denial-of-service in request handling, and a file-disclosure flaw in stylesheet source maps. Both were patch-level updates within the versions already allowed, so nothing about how the app works changes. A review confirmed the app does not use the features most of these advisories affect, so real-world exposure was limited — but they are now closed regardless, ahead of launch.
- **Launch plan enriched and a brand guideline captured (internal docs)** — Merged the two-audience go-to-market strategy, positioning and pricing into `LAUNCH_PLAN.md`, added a branded HTML rendering (`LAUNCH_PLAN.html`), and reconstructed a `BRAND.md` guideline from the app logo (the original was lost). Corrected stale pre-approval framing: the beta uses Google Calendar (the now-approved primary path), with no `.ics` fallback plan. Clarified in `CLAUDE.md` that the no-Claude-Artifacts rule bans the Artifact publish tool, not HTML files in the repo, and recorded which file owns which kind of info (backlog = technical only; launch plan = all pre-launch activity). Launch decisions mirrored into `Project_Model.md`.
...
- **Import now shows what it is actually doing** — Instead of a single "Matching…" label for the whole wait, the import shows each step in turn: reading your calendar, identifying work meetings, finding matching Jira issues, checking confidence, and preparing suggestions. Steps are described in plain language with a note on why the longer ones take time. There are deliberately no percentages or counts — the import happens in one request, so any number shown mid-way would be guesswork rather than real progress.
- **Explained how the default Jira project actually works** — The "Default Jira project key" setting is a hard limit on AI matching, not a preference: an event with no Jira key in its title can only be matched to that one project. That was never written down anywhere, and "default" implies the opposite. Settings now explains it under the field, the Help FAQ answers "Can I log time to more than one Jira project?", and the constraint is recorded in the PRD. The workaround is now discoverable too — put a Jira key in the event title (e.g. CSD-123) and it matches that ticket in any project, ahead of everything else.
- **Reused the AI connection instead of rebuilding it every import** — The app was opening a brand-new connection to the AI service on every single import; it now keeps one open and reuses it while the server stays warm. Because the app runs in Sydney and the AI service answers from the US, setting that connection up costs several trips across the Pacific. Each import also now records how much the AI actually read and wrote, so the remaining wait can be attributed to either connection setup or the model's own speed rather than guessed at.
- **Added timing detail inside the AI matching step** — AI matching is ~73% of the time an import takes, and even a single-event import spends ~1.9 seconds there, but nothing recorded *where* that time went. Each import now records a breakdown (prompt building, the AI call itself, response parsing) so the slow part can be identified rather than guessed at. Measurement only — no matching behaviour changed.
- **Fixed import analytics that had never recorded anything** — Every completed import was supposed to save a row of performance data (total time plus six per-stage timings), but the write was being rejected by a database security policy and the error was being silently discarded, so the table had sat empty since it was created in July. Imports themselves were never affected. The write now uses the correct credentials and any future failure is logged where it can be seen, so import performance can actually be diagnosed — which matters right now, because a real import measured 16.3 seconds for 72 events with 92% of that time server-side.
