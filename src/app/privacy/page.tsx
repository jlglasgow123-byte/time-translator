...
const sections = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'Where the service uses AI to generate outputs such as timesheet summaries or invoice drafts, this processing is performed by Anthropic (Claude API). Calendar event titles, descriptions, and Jira worklog data you have imported may be sent to Anthropic for this purpose. Anthropic does not use API data to train their models. No customer data is used for AI model training. If we add another AI provider in future, we will update this policy before doing so.',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'Specifically: we do not use Google user data for advertising; we do not sell Google user data; we do not transfer or disclose Google user data to others except as necessary to provide or improve user-facing features that are prominent in the app, to comply with applicable law, or as part of a merger or acquisition with notice to you; and we do not allow humans to read Google user data unless we first obtain your affirmative consent, it is necessary for security purposes (such as investigating a bug or abuse), it is required to comply with applicable law, or the data has been aggregated and anonymised so it can no longer identify an individual user.',
      'Diagnostic records containing Google user data: when a fault occurs while processing your calendar, our error records can contain a short fragment (up to 500 characters) of the affected content, which may include or paraphrase a calendar event title. This falls under the security and bug-investigation exception above — it exists solely so a fault can be diagnosed and fixed. Access is limited to our database administrator, the records are never shared or used for any other purpose, and they are automatically deleted after 90 days.',
      'Where the app sends your Google Calendar event data to Anthropic (Claude API) to generate the outputs you have requested, this transfer is limited to providing that user-facing feature. Anthropic does not use this data to train their models, and no Google user data is used for AI model training.',
  ...
  },
  {
    ...
    paragraphs: [
      ...
      'Connected Google Calendar access — when you disconnect Google Calendar, we revoke our access token with Google and delete the stored authentication tokens immediately. Your calendar events themselves are deleted when you delete your account, with one limited exception described under "Diagnostic records" below.',
      ...
      'Security and access logs — retained for up to 90 days for security monitoring, incident investigation and audit purposes, then automatically deleted.',
      'Diagnostic records — when something goes wrong (for example, the AI returns a response the app cannot read), we record what happened so the fault can be found and fixed. Because the failure concerns your data, these records can contain a short fragment — up to 500 characters — of the affected content, which may include or paraphrase a calendar event title. We store this only to diagnose faults, never to analyse your calendar or build a profile of you. These records are visible only to our database administrator; they are not shared with anyone, and are not included in any report or email. They are automatically deleted after 90 days. When you delete your account, diagnostic records are immediately unlinked from your identity, and the remaining anonymous record is deleted within 90 days.',
